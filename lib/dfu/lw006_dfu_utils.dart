...
Future<String> lw006PrepareDfuFirmwarePath(PlatformFile picked) async {
  final name = picked.name.trim();
  if (name.isEmpty || !name.toLowerCase().endsWith('.zip')) {
    throw Lw006DfuFileException('File error!');
  }
  ...
  final tempDir = Directory.systemTemp.createTempSync('lw006_dfu_');
  final fileName = name.split('/').last.split('\\').last;
  final target = File('${tempDir.path}/$fileName');
  await target.writeAsBytes(bytes, flush: true);
  debugPrint('[LW006 DFU] firmware copied to ${target.path} (${bytes.length} bytes)');
  return target.path;
}

class Lw006DfuFileException implements Exception {
  Lw006DfuFileException(this.message);
