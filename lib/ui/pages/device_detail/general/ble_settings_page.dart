...
import 'package:lw006_flutter/ble/lw006_device_session.dart';
import 'package:lw006_flutter/ble/lw006_option_lists.dart';
import 'package:lw006_flutter/ble/lw006_param_helpers.dart';
import 'package:lw006_flutter/ble/lw006_protocol_named_api.dart';
import 'package:lw006_flutter/ui/widgets/ble_change_password_dialog.dart';
import 'package:lw006_flutter/ui/widgets/ble_loading_overlay.dart';
import 'package:lw006_flutter/ui/widgets/device_detail/settings_widgets.dart';
...
class _BleSettingsPageState extends State<BleSettingsPage> {
  ...
  Future<void> _load() async {
    await runWithBleLoading(context, () async {
      ...
      if (!mounted) return;
      _advName.text = Lw006ParamHelpers.bytesToString(results[0].data);
      _interval.text = Lw006ParamHelpers.uint8(results[1].data).toString();
      final txPower = Lw006ParamHelpers.byte0(results[2].data);
      _txPowerIndex = Lw006OptionLists.txPowerLevels.indexOf(txPower);
      if (_txPowerIndex < 0) _txPowerIndex = 6;
      _timeout.text = Lw006ParamHelpers.uint8(results[3].data).toString();
      _passwordVerify = Lw006ParamHelpers.uint8(results[4].data) == 1;
  ...
  Future<void> _save() async {
    ...
    await runWithBleLoading(context, () async {
      ...
      final ok = (await Future.wait([
        ...
        api.writeAdvTxPower([Lw006OptionLists.txPowerLevels[_txPowerIndex]]),
      ...
      body: ListView(
        ...
        children: [
          ...
          SettingsCard(
            child: SettingsLabelRow(
              label: 'ADV Interval',
              child: SettingsTextField(controller: _interval, hint: '1~100', suffix: '100ms'),
            ),
          ),
          SettingsCard(
            child: SettingsLabelRow(
              label: 'ADV Timeout',
              child: SettingsTextField(controller: _timeout, hint: '1~60', suffix: 's'),
            ),
          ),
          SettingsCard(
            child: SettingsSliderRow(
              ...
              min: 0,
              max: (Lw006OptionLists.txPowerLevels.length - 1).toDouble(),
              suffix: '${Lw006OptionLists.txPowerLevels[_txPowerIndex]}dBm',
