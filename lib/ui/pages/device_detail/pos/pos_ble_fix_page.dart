...
class _PosBleFixPageState extends State<PosBleFixPage> {
  ...
  Future<void> _load() async {
    await runWithBleLoading(context, () async {
      ...
      _rssi = Lw006ParamHelpers.byte0(results[3].data, defaultValue: -127).toDouble();
      _relationshipIndex = Lw006ParamHelpers.uint8(results[4].data).clamp(0, 6);
      setState(() {});
    });
  }
  ...
  Future<void> _save() async {
    ...
    if (timeout == null || timeout < 1 || timeout > 10 || macNum == null || macNum < 1 || macNum > 5) {
    ...
    return DetailScaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          SettingsCard(child: SettingsLabelRow(label: 'MAC Number', child: SettingsTextField(controller: _macNumber, hint: '1~5'))),
          SettingsCard(child: SettingsLabelRow(label: 'BLE Fix Mechanism', child: BlueValueButton(text: Lw006OptionLists.bleFixMechanism[_mechanismIndex], onTap: () async {
            final i = await showBottomPicker(context: context, options: Lw006OptionLists.bleFixMechanism, selectedIndex: _mechanismIndex);
          ...
          SettingsCard(child: SettingsLabelRow(label: 'Filter Relationship', child: BlueValueButton(text: Lw006OptionLists.filterRelationship[_relationshipIndex], onTap: () async {
            final i = await showBottomPicker(context: context, options: Lw006OptionLists.filterRelationship, selectedIndex: _relationshipIndex);
