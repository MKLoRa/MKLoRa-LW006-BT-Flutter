...
class _MessageTypeSettingsPageState extends State<MessageTypeSettingsPage> {
  ...
  Future<void> _load() async {
    await runWithBleLoading(context, () async {
      for (final row in _rows) {
        ...
        final cfg = Lw006PayloadConfig.fromBytes(result.data);
  ...
  Future<void> _pickType(_PayloadRow row) async {
    final index = await showBottomPicker(
      context: context,
      options: Lw006OptionLists.payloadTypes,
      selectedIndex: row.confirmed ? 1 : 0,
    );
  ...
  Future<void> _pickRetrans(_PayloadRow row) async {
    final index = await showBottomPicker(
      context: context,
      options: Lw006OptionLists.retransmissionTimes,
      selectedIndex: row.retransIndex,
    );
  ...
  Future<void> _save() async {
    await runWithBleLoading(context, () async {
      final results = await Future.wait(_rows.map((r) => r.write(
            Lw006PayloadConfig(confirmed: r.confirmed, retransIndex: r.retransIndex).toBytes(),
          )));
                ...
                children: [
                  ...
                  if (row.confirmed) ...[
                    ...
                    SettingsLabelRow(
                      ...
                      child: BlueValueButton(
                        text: Lw006OptionLists.retransmissionTimes[row.retransIndex],
